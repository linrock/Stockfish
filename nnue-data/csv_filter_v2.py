...
if input_filename.endswith(".csv"):
    output_filename = input_filename.replace('.csv', '.csv.filter-v2.plain')
elif input_filename.endswith(".csv.zst"):
    output_filename = input_filename.replace('.csv.zst', '.csv.zst.filter-v2.plain')

if os.path.isfile(output_filename):
    print(f'Found .csv.zst.filter-v2.plain file, doing nothing:')
    print(output_filename)
    sys.exit(0)
elif os.path.isfile(output_filename.replace('.csv.zst.filter-v2.plain', '.csv.zst.filter-v2.binpack')):
    print(f'Found .csv.zst.filter-v2.binpack file, doing nothing:')
    print(output_filename.replace('.csv.zst.filter-v2.plain', '.csv.zst.filter-v2.binpack'))
    sys.exit(0)
                ...
                if b.is_check():
                    ...
                    if b.is_capture(bestmove):
                        ...
                        if b.is_capture(sf_bestmove1) or move_is_promo(sf_bestmove1_uci):
                            # skip if SF search 1st best move is a capture
                            ...
                            if b.is_capture(sf_bestmove2) or move_is_promo(sf_bestmove2_uci):
                                # skip if SF search 2nd best move is a capture
                                self.num_sf_bestmove2_captures += 1
                                should_filter_out = True
                            else:
