...
namespace {
  ...
  int tFutPruneChildDepth = 8;
  int tFutPruneDenom = 304;
  int tFutPruneEvalThresh = 28580;
  TUNE(SetRange(4, 12), tFutPruneChildDepth);
  TUNE(SetRange(100, 500), tFutPruneDenom);
  TUNE(SetRange(20000, 36000), tFutPruneEvalThresh);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        &&  depth < tFutPruneChildDepth
        &&  eval - futility_margin(depth, improving) - (ss-1)->statScore / tFutPruneDenom >= beta
        &&  eval >= beta
        &&  eval < tFutPruneEvalThresh) // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;
