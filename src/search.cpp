...
namespace Stockfish {
  ...
  int TUNE_histDepthMult = -3645;
  int TUNE_fpcEvalOffset = 239;
  int TUNE_fpcLmrDepthMult = 291;
  int TUNE_histDenom = 7836;
  int TUNE_fpEvalOffset = 77;
  int TUNE_fpHistThresh = 989;
  int TUNE_futBaseOffset = 200;

  TUNE(SetRange(0, 394), TUNE_fpcEvalOffset);
  TUNE(SetRange(0, 412), TUNE_fpcLmrDepthMult);
  TUNE(SetRange(-7290, -1822), TUNE_histDepthMult);
  TUNE(SetRange(2836, 11536), TUNE_histDenom);
  TUNE(SetRange(0, 154), TUNE_fpEvalOffset);
  TUNE(SetRange(0, 1978), TUNE_fpHistThresh);
  TUNE(SetRange(0, 400), TUNE_futBaseOffset);
        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    ...
                    int   futilityEval =
                      ss->staticEval + TUNE_fpcEvalOffset + TUNE_fpcLmrDepthMult * lmrDepth + PieceValue[capturedPiece]
            ...
            else
            {
                ...
                if (!ss->inCheck && lmrDepth < 13 && ss->staticEval + TUNE_fpEvalOffset + 124 * lmrDepth <= alpha)
                    continue;
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        ...
        futilityBase = ss->staticEval + TUNE_futBaseOffset;
    }
