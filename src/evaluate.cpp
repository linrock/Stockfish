...
bool Eval::use_smallnet(const Position& pos) {
    ...
    return std::abs(simpleEval) > 1138 + 6 * pos.count<PAWN>();
}
...
Value Eval::evaluate(const Eval::NNUE::Networks&    networks,
    ...
    if (smallNet && (nnue * simpleEval < 0 || std::abs(nnue) < 491))
    {
    ...
    const auto adjustEval = [&](int shufflingConstant) {
        ...
        nnue -= nnue * (nnueComplexity * 5 / 3) / 31618;

        int npm = pos.non_pawn_material() / 64;
        v       = (nnue * (npm + 949 + 11 * pos.count<PAWN>()) + optimism * (npm + 140)) / 1041;
