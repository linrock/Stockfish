...
namespace {
  ...
  constexpr Value LazyThreshold1    =  Value(3538);
  constexpr Value LazyThreshold2    =  Value(1976);
  constexpr Value SpaceThreshold    =  Value(11502);
...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  bool useClassical = !useNNUE || (pos.count<ALL_PIECES>() > 7 && abs(psq) > 1830);
  ...
  else
  {
      ...
      nnueComplexity = (  389 * nnueComplexity
                        + 424 * abs(psq - nnue)
      ...
      optimism = optimism * (271 + nnueComplexity) / 256;
      v = (nnue * scale + optimism * (scale - 791)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (207 - pos.rule50_count()) / 244;
