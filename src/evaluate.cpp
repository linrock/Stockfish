...
namespace {
  ...
  constexpr Value LazyThreshold1    =  Value(3493);
  constexpr Value LazyThreshold2    =  Value(2036);
  constexpr Value SpaceThreshold    =  Value(11697);
...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  bool useClassical = !useNNUE || (pos.count<ALL_PIECES>() > 7 && abs(psq) > 1824);
  ...
  else
  {
      ...
      nnueComplexity = (  367 * nnueComplexity
                        + 424 * abs(psq - nnue)
      ...
      optimism = optimism * (275 + nnueComplexity) / 256;
      v = (nnue * scale + optimism * (scale - 821)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (220 - pos.rule50_count()) / 244;
