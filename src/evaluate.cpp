...
bool Eval::use_smallnet(const Position& pos) {
    ...
    return std::abs(simpleEval) > 982 + 6 * pos.count<PAWN>();
}
...
Value Eval::evaluate(const Eval::NNUE::Networks&    networks,
    ...
    nnue -= nnue * (nnueComplexity * 5 / 3) / 31930;

    int material = 200 * pos.count<PAWN>() + 350 * pos.count<KNIGHT>() + 400 * pos.count<BISHOP>()
                 + 640 * pos.count<ROOK>() + 1200 * pos.count<QUEEN>();

    v = (nnue * (34000 + material + 256 * pos.count<PAWN>())
         + optimism * (4400 + material + 16 * pos.count<PAWN>()))
      / 35420;
