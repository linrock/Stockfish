...
namespace Stockfish {
  ...
  constexpr int TUNE_nnueScaleBase = 926;
  constexpr int TUNE_nnueScalePawnMult = 45;
  constexpr int TUNE_nnueScaleNpMult = 24;

  constexpr int TUNE_optScaleBase = 199;
  constexpr int TUNE_optScalePawnMult = 19;
  constexpr int TUNE_optScaleNpMult = 30;

  constexpr int TUNE_nnueComplexityOptOffset = 485;
...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      int nnueScale = TUNE_nnueScaleBase
                    + TUNE_nnueScalePawnMult * pos.count<PAWN>() / 4
                    + TUNE_nnueScaleNpMult * pos.non_pawn_material() / 2048;
      int optScale  = TUNE_optScaleBase
                    + TUNE_optScalePawnMult * pos.count<PAWN>() / 4
