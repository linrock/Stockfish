...
namespace {
  ...
  constexpr Value LazyThreshold1    =  Value(3859);
  constexpr Value LazyThreshold2    =  Value(1997);
  constexpr Value SpaceThreshold    =  Value(11757);
...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  bool useClassical = !useNNUE || (pos.count<ALL_PIECES>() > 7 && abs(psq) > 1792);
  ...
  else
  {
      ...
      nnueComplexity = (  379 * nnueComplexity
                        + 424 * abs(psq - nnue)
      ...
      optimism = optimism * (264 + nnueComplexity) / 256;
      v = (nnue * scale + optimism * (scale - 785)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (206 - pos.rule50_count()) / 228;
