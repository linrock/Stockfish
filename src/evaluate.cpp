...
namespace Stockfish {
      ...
      TUNE(SetRange(1662, 2262), TUNE_LazyThreshold2);

      int TUNE_psqThresh = 2048;
      int TUNE_nnueComplexMult = 25;
      int TUNE_npmOffset = 945;
      int TUNE_optOffset = 174;
      TUNE(SetRange(1848, 2248), TUNE_psqThresh);
      TUNE(SetRange(0, 50), TUNE_nnueComplexMult);
...
Value Eval::evaluate(const Position& pos) {
  ...
  bool useClassical = !useNNUE || (pos.count<ALL_PIECES>() > 3 && abs(psq) > TUNE_psqThresh);
  ...
  else
  {
      ...
      nnueComplexity = TUNE_nnueComplexMult * (nnueComplexity + abs(psq - nnue)) / 64;
