...
bool Eval::use_smallnet(const Position& pos) {
    ...
    return std::abs(simpleEval) > 1143 + 5 * pos.count<PAWN>();
}
...
Value Eval::evaluate(const Eval::NNUE::Networks&    networks,
    ...
    if (smallNet && (nnue * simpleEval < 0 || std::abs(nnue) < 497))
    {
    ...
    const auto adjustEval = [&](int shufflingConstant) {
        ...
        optimism += optimism * (nnueComplexity + std::abs(simpleEval - nnue)) / 627;
        nnue -= nnue * (nnueComplexity * 5 / 3) / 31014;

        int npm = pos.non_pawn_material() / 64;
        v       = (nnue * (npm + 955 + 11 * pos.count<PAWN>()) + optimism * (npm + 140)) / 1019;
