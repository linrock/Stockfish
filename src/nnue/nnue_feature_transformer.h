    ...
    std::int32_t transform(const Position& pos, OutputType* output, int bucket) const {
        ...
        for (IndexType p = 0; p < 2; ++p)
        {
            ...
            for (int j = 0; j < NumOutputChunks; ++j)
            {
                ...
                // std::int16_t v = min(abs(in[j]), 127) - 127;
            ...
            for (IndexType j = 0; j < HalfDimensions; ++j)
            {
                ...
                std::int16_t v   = std::max(127 - std::abs(sum), 0);
   ...
   private:
    void update_accumulator(const Position& pos, const Color perspective) const {
        ...
        if (st->accumulator.computed[perspective])
        {
            ...
            FeatureSet::append_changed_indices(ksq, next->dirtyPiece, perspective, removed[0],
                                               added[0]);
            for (StateInfo* st2 = pos.state(); st2 != next; st2 = st2->previous)
                FeatureSet::append_changed_indices(ksq, st2->dirtyPiece, perspective, removed[1],
                                                   added[1]);
