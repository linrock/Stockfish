...
class FeatureTransformer {
   ...
   public:
    ...
    inline void scale_weights(bool read) const {
        if (TransformedFeatureDimensions == TransformedFeatureDimensionsSmall)
            return;
    ...
    std::int32_t transform(const Position&                           pos,
        ...
        for (IndexType p = 0; p < 2; ++p)
        {
            ...
            const vec_t One  = vec_set_16(TransformedFeatureDimensions == TransformedFeatureDimensionsBig ? 127 * 2 : 255);
            ...
            for (IndexType j = 0; j < HalfDimensions / 2; ++j)
            {
                ...
                sum0               = std::clamp<BiasType>(sum0, 0, One);
                sum1               = std::clamp<BiasType>(sum1, 0, One);
