...
// Definition of layer AffineTransformSparseInputx2 of NNUE evaluation function

#ifndef NNUE_LAYERS_AFFINE_TRANSFORM_SPARSE_INPUT_X2_H_INCLUDED
#define NNUE_LAYERS_AFFINE_TRANSFORM_SPARSE_INPUT_X2_H_INCLUDED
...
# undef vec_nnz
  ...
  class AffineTransformSparseInputx2 {
   public:
    // Input/output type
    // Input/output type
    ...
    static_assert(OutputDimensions % 32 == 0, "Only implemented for OutputDimensions divisible by 32.");
    ...
    const OutputType* propagate(
      ...
      std::uint16_t nnz[4][NumChunks / 4];
      IndexType count[4];
      ...
      for (IndexType i = 0; i < 4; ++i)
        find_nnz<NumChunks / 4>(input32 + i * NumChunks / 4, nnz[i], count[i]);
      ...
      {
        IndexType quarter = p * 2;
        for (IndexType j = 0; j < count[quarter]; ++j)
        {
          const auto i = nnz[quarter][j] + quarter * NumChunks / 4;
          const vec_t in = vec_set_32(input32[i]);
          const auto col = reinterpret_cast<const vec_t*>(&weights[i * OutputDimensions * ChunkSize]);
          for (IndexType k = NumRegs / 2; k < NumRegs; ++k)
...
#endif // #ifndef NNUE_LAYERS_AFFINE_TRANSFORM_SPARSE_INPUT_X2_H_INCLUDED
